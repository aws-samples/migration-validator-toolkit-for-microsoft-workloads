<#
  .Synopsis
    Check the boot mode of the source machine.
  .Description
    Check the boot mode of the source machine (Legacy BIOS or UEFI) and provide a status message.
  .Example
    Get-BootMode
  .OUTPUTS
...
Function Get-BootMode {
  ...
  try {
    ...
    $bootpartition = Get-WmiObject Win32_DiskPartition | Where-Object { $_.BootPartition -eq "True" }
    ...
    (Get-Content $tempDirectory\DiskPartOutput.txt) | ? { $_.trim() -ne "" } | Select-Object -skip 5 | Set-Content $tempDirectory\DiskPartOutput.txt
